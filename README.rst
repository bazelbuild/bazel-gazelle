...
Gazelle also contains directives that it reads from the WORKSPACE file.
It applies these directives when it reads repository rules declared in
WORKSPACE in order to discover custom repository names and known prefixes.
``fix-update`` uses this information for dependency resolution, and
``update-repos`` uses it to know about the repos already defined.

+---------------------------------------------------+----------------------------------------+
| **WORKSPACE Directive**                           | **Default value**                      |
+===================================================+========================================+
| :direc:`# gazelle:repository_macro`               | n/a                                    |
+---------------------------------------------------+----------------------------------------+
| Tells Gazelle to look for repository rules in a macro in a .bzl file. The directive can be |
| repeated multiple times and has the following format:                                      |
|                                                                                            |
