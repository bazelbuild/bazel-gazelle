...
func (gl *goLang) GenerateRules(args language.GenerateArgs) language.GenerateResult {
	...
	// This is a collection of go_proto_library rules that were generated by
	// another gazelle rule.
	//
	//   key: proto_library rule name (colon-prefixed)
	//   value: go_proto_library rule
	goProtoRules := make(map[string]*rule.Rule)
	var protoRuleNames []string
	protoPackages := make(map[string]proto.Package)
	protoFileInfo := make(map[string]proto.FileInfo)
	for _, r := range args.OtherGen {
		if r.Kind() == "go_proto_library" {
			goProtoRules[r.AttrString("proto")] = r
		}
		...
		if _, ok := err.(*build.NoGoError); ok {
			if len(protoPackages) == 1 {
				for name, ppkg := range protoPackages {
					...
					if r, ok := goProtoRules[":"+name]; ok {
						// if a go_proto_library rule already exists for this
						// proto package, use the import path given in the rule
						pkg.importPath = r.AttrString("importpath")
					}
					protoName = name
					break
				}
	...
	for _, name := range protoRuleNames {
		if r := goProtoRules[":"+name]; r != nil {
			...
			if pkg != nil && r.AttrString("importpath") == pkg.importPath {
				// if import paths match, embed the preexisting go_proto_library
				// to the go rule to be created.
				protoEmbed = r.Name()
			}
			continue
		}
